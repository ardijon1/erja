...
export async function GET(
  ...
  if (!rateLimit(`ref:${ip}`, 60, 60 * 1000)) {
    // Rate-limited but still redirect silently to /
  ...
  // Invalid / empty → silent redirect to /
  if (!trimmed) {
    return NextResponse.redirect(new URL("/", request.url), 302);
  }

  // Validate + log click (returns null if code not found)
  ...
  // Valid → set signed referral cookie (first-touch wins) and render landing page
